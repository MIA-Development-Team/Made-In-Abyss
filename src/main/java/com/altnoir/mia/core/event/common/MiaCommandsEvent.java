...
public class MiaCommandsEvent {
    ...
    private static int setMaxCurse(
            CommandContext<CommandSourceStack> ctx, ServerPlayer player, int value) {
    ...
    private static int clearCurse(CommandContext<CommandSourceStack> ctx, ServerPlayer player) {
        ...
        ctx.getSource()
                .sendSuccess(
                        () -> Component.literal("Cleared curse of " + player.getName().getString()),
                        true);
        return 1;
    }
