...
public class HookItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(
        ...
        if (customData.contains("hook")
                && level.getEntity(customData.copyTag().getInt("hook"))
                        instanceof HookEntity entity) {
            // 尝试优化手感，当距离小于回收距离时，删除原来抓钩，立即发射,不需收回
            if (entity.distanceToSqr(player)
                    > MiaConfig.hookRetractDistance * MiaConfig.hookRetractDistance) {
                level.playSound(
                        null,
                        player.getX(),
                        player.getY(),
