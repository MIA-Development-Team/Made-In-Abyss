...
public class GreatFaultPlacements {
    ...
    private static final PlacementModifier GREAT_FAULT_HEIGHT =
            HeightRangePlacement.uniform(
                    VerticalAnchor.aboveBottom(16), VerticalAnchor.belowTop(16));

    public static void bootstrap(BootstrapContext<PlacedFeature> context) {
        HolderGetter<ConfiguredFeature<?, ?>> holdergetter =
                context.lookup(Registries.CONFIGURED_FEATURE);

        Holder<ConfiguredFeature<?, ?>> abyss_light =
                holdergetter.getOrThrow(GreatFaultFeatures.ABYSS_LIGHT);
        Holder<ConfiguredFeature<?, ?>> caerulite_geode =
                holdergetter.getOrThrow(GreatFaultFeatures.CAERULITE_GEODE);
