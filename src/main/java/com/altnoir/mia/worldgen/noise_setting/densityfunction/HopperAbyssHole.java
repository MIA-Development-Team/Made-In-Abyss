...
public class HopperAbyssHole extends AbstractAbyssHole {
    ...
    public HopperAbyssHole(float radius) {
        super(radius);
    }
    ...
    private static float getHeightValue(int x, int y, int z, float r) {
        float d = Mth.sqrt((float) (x * x + z * z));

        float maxY = 2.0F;
        float yr = Mth.clamp(1.0F + (float) y / 64.0F, 1.0F, maxY); // y除的数越小，坡度就越平滑

        float f = ((getAbyssRadius() + r) / maxY) * yr - d * 8.0F;
    ...
    public double compute(DensityFunction.FunctionContext context) {
        ...
        return (8.0 - (double) getHeightValue(context.blockX() / 8, context.blockY() / 8, context.blockZ() / 8, this.radius)) / 64; // 最终除的数越小，洞越平滑
    }
