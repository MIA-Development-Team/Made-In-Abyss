...
public record CaveFloorSearch(VerticalAnchor minY, int clearance, int horizontalSamples) {
    public static final Codec<CaveFloorSearch> CODEC = RecordCodecBuilder.create(instance -> instance.group(
    ...
    public Optional<BlockPos> findStart(Structure.GenerationContext context, int maxStartY) {
        ...
        int minFloorY = Math.max(this.minY.resolveY(heightContext), context.heightAccessor().getMinY());
        int boundedMaxStartY = Math.min(
                maxStartY,
                context.heightAccessor().getMaxY() + 1 - this.clearance
        );
