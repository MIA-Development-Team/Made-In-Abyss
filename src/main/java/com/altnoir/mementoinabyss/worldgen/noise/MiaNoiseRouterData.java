...
public class MiaNoiseRouterData extends NoiseRouterData {
    ...
    private static DensityFunction greatFaultDensity(HolderGetter<DensityFunction> densityFunctions) {
        ...
        DensityFunction rangeChoice = DensityFunctions.rangeChoice(
                yFunction, MiaHeight.GREAT_FAULT.minY(), MiaHeight.GREAT_FAULT.maxY() - 64, abyssHoleBelow, abyssHoleAbove
        );
    ...
    private static NoiseRouter theAbyssRouter(HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParameters) {
        ...
        DensityFunction idwj1 = DensityFunctions.rangeChoice(yFunction, -128, 5, MiaDensityFunctionTypes.general(0.0F, 0.25F), MiaDensityFunctionTypes.hopper());
        ...
        DensityFunction combinedDensity = DensityFunctions.max(
                DensityFunctions.min(theAbyssDensity(densityFunctions), noodle), greatCaveNoodle);
        DensityFunction finalDensity = DensityFunctions.rangeChoice(
                yFunction, -190, MiaHeight.THE_ABYSS.maxY(),
                combinedDensity, DensityFunctions.constant(-64.0));
