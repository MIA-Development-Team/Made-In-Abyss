...
import com.altnoir.mementoinabyss.init.MiaDensityFunctionTypes;
...
public class MiaDensityFunctions {
    ...
    private static ResourceKey<DensityFunction> createKey(String location) {
        return ResourceKey.create(Registries.DENSITY_FUNCTION, MementoInAbyss.asResource(location));
    }
    ...
    public static void bootstrap(BootstrapContext<DensityFunction> context) {
        ...
        context.register(THE_ABYSS_DEPTH, DensityFunctions.rangeChoice(
                yFunction, MiaHeight.THE_ABYSS.minY(), 0,
                DensityFunctions.rangeChoice(
                        yFunction, MiaHeight.THE_ABYSS.minY(), -112,
                        DensityFunctions.constant(2.0), DensityFunctions.constant(1.9)
                ), DensityFunctions.yClampedGradient(MiaHeight.THE_ABYSS.middleY(), 832, 1.5, -2.5)));

        context.register(THE_ABYSS_HOLE_ABOVE, DensityFunctions.add(MiaDensityFunctionTypes.hopper(), getFunction(holdergetter1, BASE_3D_NOISE_THE_ABYSS)));
        context.register(THE_ABYSS_HOLE_BELOW, DensityFunctions.add(MiaDensityFunctionTypes.general(0.0F, 0.3F), getFunction(holdergetter1, BASE_3D_NOISE_THE_ABYSS)));
        context.register(THE_ABYSS_BIG_HOLE, DensityFunctions.add(MiaDensityFunctionTypes.general(128.0F), getFunction(holdergetter1, BASE_3D_NOISE_THE_ABYSS)));
        context.register(THE_ABYSS_INSIDE_HOLE_BELOW, abyssDensityFunction(holdergetter1, THE_ABYSS_HOLE_BELOW, MiaHeight.THE_ABYSS.minY() + 128, MiaHeight.THE_ABYSS.middleY()));
        context.register(THE_ABYSS_INSIDE_HOLE_ABOVE, abyssDensityFunction(holdergetter1, THE_ABYSS_HOLE_ABOVE, MiaHeight.THE_ABYSS.middleY(), MiaHeight.THE_ABYSS.maxY() - 64));
        ...
        context.register(THE_ABYSS_GREAT_CAVE_NOODLE, DensityFunctions.rangeChoice(
                DensityFunctions.min(
                        ...
                        MiaDensityFunctionTypes.noodle(0.0F, 0.2F)
                ),
        ...
        // Layer 3
        context.register(GREAT_FAULT_HOLE_ABOVE, DensityFunctions.add(MiaDensityFunctionTypes.hopper(0.0F, 0.6F, 16.0F), getFunction(holdergetter1, BASE_3D_NOISE_THE_ABYSS)));
        context.register(GREAT_FAULT_HOLE_BELOW, DensityFunctions.add(MiaDensityFunctionTypes.general(0.0F, 0.5F), getFunction(holdergetter1, BASE_3D_NOISE_THE_ABYSS)));
    ...
    private static DensityFunction theAbyssGreatCaveNoodle(HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParameters) {
        return abyssNoodle(MiaHeight.THE_ABYSS.minY() + 64, 5, densityFunctions, noiseParameters, 0.5, 0.3, 0.2, true);
    }

    private static DensityFunction abyssNoodle(HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParameters) {
        return abyssNoodle(MiaHeight.THE_ABYSS.minY() + 128, MiaHeight.THE_ABYSS.maxY(), densityFunctions, noiseParameters, 1.5, -0.5, -0.3, 40, false);
    }
    ...
    private static DensityFunction greatFaultNoodle(HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParameters) {
        return abyssNoodle(MiaHeight.GREAT_FAULT.minY(), MiaHeight.GREAT_FAULT.maxY(), densityFunctions, noiseParameters, 2.0, 1.0, -0.1, -0.15, false);
    }

    private static DensityFunction abyssNoodle(
            int minY, int maxY, HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParameters, double noiseScale, double fromY, double toY, boolean inverse
    ) {
        return abyssNoodle(minY, maxY, densityFunctions, noiseParameters, noiseScale, fromY, toY, 0, inverse);
    }

    private static DensityFunction abyssNoodle(
            int minY, int maxY, HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParameters, double noiseScale, double fromY, double toY, int YOffset, boolean inverse
    ) {
        return abyssNoodleBase(minY, maxY, densityFunctions, noiseParameters, noiseScale, noiseScale, 2.6666666666666665, fromY, toY, YOffset, true, inverse, false);
    }

    private static DensityFunction abyssNoodle(
            int minY, int maxY, HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParameters, double noiseScaleX, double noiseScaleY, double fromY, double toY, boolean inverse
    ) {
        return abyssNoodleBase(minY, maxY, densityFunctions, noiseParameters, noiseScaleX, noiseScaleY, 0.5, fromY, toY, 0, true, inverse, true);
    }
    ...
    private static DensityFunction abyssNoodleBase(
            ...
            double noiseScaleX, double noiseScaleY, double d0, double fromY, double toY, int YOffset, boolean ridgeB, boolean inverse, boolean full
    ) {
        ...
        DensityFunction densityFunction0 = yLimitedInterpolatable(
                yFunction, DensityFunctions.noise(noiseParameters.getOrThrow(Noises.NOODLE), noiseScaleX, noiseScaleY), FMinY, FMaxY, -1
        );
        // 剃头（看情况再添加）
        DensityFunction densityFunction1 = DensityFunctions.add(
                DensityFunctions.rangeChoice(
                        yFunction, minY, maxY - 64, DensityFunctions.constant(0), DensityFunctions.constant(1)
                ), densityFunction0
        );
        ...
        DensityFunction densityFunction3 = yLimitedInterpolatable(
                yFunction, DensityFunctions.noise(noiseParameters.getOrThrow(Noises.NOODLE_RIDGE_A), d1, d0), minY, maxY, inverse ? -1 : 0
        );

        DensityFunction densityFunction4 = yLimitedInterpolatable(
                yFunction, DensityFunctions.noise(noiseParameters.getOrThrow(Noises.NOODLE_RIDGE_B), d1, d0), minY, maxY, inverse ? -1 : 0
        );

        DensityFunction densityFunction5 = DensityFunctions.mul(DensityFunctions.constant(inverse ? -1.5 : 1.5), densityFunction3.abs());
        DensityFunction densityFunction6 = DensityFunctions.mul(DensityFunctions.constant(inverse ? -1.5 : 1.5), DensityFunctions.max(densityFunction3.abs(), densityFunction4.abs()));
    ...
    private static DensityFunction bigCaves(HolderGetter<NormalNoise.NoiseParameters> noiseParameters) {
        ...
        DensityFunction densityfunction1 = DensityFunctions.add(
                ...
                DensityFunctions.mul(
                        DensityFunctions.noise(noiseParameters.getOrThrow(Noises.PATCH), 0.0, 1.0),
                        DensityFunctions.constant(0.5).cube()
                )
        );
        DensityFunction densityfunction2 = DensityFunctions.cacheOnce(DensityFunctions.add(densityfunction, densityfunction1));
