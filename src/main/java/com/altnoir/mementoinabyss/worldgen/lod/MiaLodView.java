...
public record MiaLodView(double x, double y, double z, double pixelScale, List<Plane> planes) {
    ...
    public MiaLodView {
        if (!Double.isFinite(x) || !Double.isFinite(y) || !Double.isFinite(z)
                ...
                || !Double.isFinite(pixelScale) || pixelScale < 1 || pixelScale > 16384
                || planes == null || planes.size() != 4) throw new IllegalArgumentException("Invalid LOD camera");
    ...
    public record Plane(float x, float y, float z, float w) {
        public Plane {
            ...
            if (!Double.isFinite(length) || length < .99 || length > 1.01 || !Float.isFinite(w) || Math.abs(w) > 65536) {
                throw new IllegalArgumentException("Invalid LOD camera plane");
            }
        }
    }

    public double distanceSquared(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
    ...
    public boolean visible(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, double margin) {
        for (Plane p : planes) {
    ...
    public int cellSize(double distanceSquared, int previous) {
        ...
        int desired = 16 * scale <= TARGET_CELL_PIXELS ? 16 : 4 * scale <= TARGET_CELL_PIXELS ? 4 : 1;
    ...
    public double priority(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
    ...
    public boolean materiallyDifferent(MiaLodView other) {
        if (other == null || squared(x - other.x, y - other.y, z - other.z) >= 4
                || Math.abs(pixelScale - other.pixelScale) > pixelScale * .03) return true;
        for (int i = 0; i < 4; i++) {
            Plane a = planes.get(i), b = other.planes.get(i);
            if (squared(a.x - b.x, a.y - b.y, a.z - b.z) > .0004 || Math.abs(a.w - b.w) > 1) return true;
        }
        return false;
    }
