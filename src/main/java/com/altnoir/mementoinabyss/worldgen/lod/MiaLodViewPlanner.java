...
public final class MiaLodViewPlanner {
    ...
    public static List<Demand> plan(MiaLodView view, int radius, int minY, int maxY, LongToIntFunction confirmedLevel) {
        if (radius <= 0 || radius > 2048 || minY >= maxY) throw new IllegalArgumentException("Invalid LOD view window");
        int centerX = (int) Math.floor(view.x() / 16), centerZ = (int) Math.floor(view.z() / 16);
        int chunks = (radius + 15) / 16 + 1; // Include columns whose far face just touches the sphere.
        ...
        for (int x = centerX - chunks; x <= centerX + chunks; x++) {
            for (int z = centerZ - chunks; z <= centerZ + chunks; z++) {
                ...
                double distance = view.distanceSquared(minX, minY, minZ, minX + 16, maxY, minZ + 16);
                ...
                int cell = previous == 0 ? 16 : band == 2 ? previous : view.cellSize(distance, previous);
                // Finish nearby detail without waiting for a whole-radius coarse sweep. Missing cover wins
                // within each 64-block shell; the send ledger still reserves credits for new visible cover.
                double priority = band * 1e12 + Math.floor(Math.sqrt(distance) / 64) * 1e8
                        + (previous == 0 ? 0 : 1e6) + distance;
    ...
    static boolean preemptUnsent(int sectionsSent, int cellSize, Demand previous, Demand next) {
        ...
        return next == null || next.cellSize() > cellSize
