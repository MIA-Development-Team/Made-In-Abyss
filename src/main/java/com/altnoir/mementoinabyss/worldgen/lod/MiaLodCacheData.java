...
public record MiaLodCacheData(int cellSize, int minY, int sectionCount, String[] palette, short[] voxels) {
    ...
    public MiaLodCacheData {
        ...
        if (voxels.length != sectionCount * size * size * size) throw new IllegalArgumentException("Invalid cached LOD size");
        Set<String> seen = new HashSet<>();
        for (int i = 0; i < palette.length; i++) {
            String state = palette[i];
            if (state == null || state.isBlank() || state.length() > 1024 || !seen.add(state)
                    || i > 1 && palette[i - 1].compareTo(state) >= 0) throw new IllegalArgumentException("Invalid cached LOD palette");
        }
        ...
        for (short voxel : voxels) {
            ...
            if (index >= palette.length) throw new IllegalArgumentException("Invalid cached LOD index");
            used[index] = true;
        }
        for (boolean value : used) if (!value) throw new IllegalArgumentException("Non-canonical cached LOD palette");
    }

    private static void validateLayout(int cellSize, int minY, int sectionCount) {
        if (cellSize < 1 || cellSize > 16 || Integer.bitCount(cellSize) != 1 || minY % 16 != 0
                || minY < -65536 || minY > 65536 || sectionCount < 1 || sectionCount > 1024) {
            throw new IllegalArgumentException("Invalid cached LOD layout");
        }
        int size = 16 / cellSize;
    ...
    public static MiaLodCacheData fromSections(int cellSize, int minY, List<Section> sections, IntFunction<String> names) {
        ...
        for (int i = 0; i < sections.size(); i++) {
            ...
            if (section.sectionY() != i || section.voxels().length != volume
                    || !air.equals(names.apply(section.palette()[0]))) throw new IllegalArgumentException("Inconsistent cache sections");
            boolean[] used = new boolean[section.palette().length];
            for (short voxel : section.voxels()) used[Short.toUnsignedInt(voxel)] = true;
            for (int p = 0; p < used.length; p++) if (used[p]) {
    ...
    public List<Section> sections(CrossDimensionLodTransfer t, ToIntFunction<String> resolve) {
        if (!t.replacement() || !matchesLayout(t)) throw new IllegalArgumentException("Incompatible cached LOD layout");
        ...
        for (int i = 0; i < sectionCount; i++) {
            ...
            result.add(new Section(i, t.targetRevision(), (byte) 63, Arrays.copyOf(localPalette, used), data));
        }
    ...
    public String digest() {
        ...
        for (int value : new int[]{FORMAT, cellSize, minY, sectionCount, palette.length}) update(digest, value);
