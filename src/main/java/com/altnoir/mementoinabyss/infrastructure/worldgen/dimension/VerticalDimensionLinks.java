...
public final class VerticalDimensionLinks {
    public static final VerticalDimensionLink ABYSS_ABOVE_GREAT_FAULT =
            new VerticalDimensionLink(
    ...
    private static final CopyOnWriteArrayList<VerticalDimensionLink> LINKS =
    ...
    public static void register(VerticalDimensionLink link) {
        if (below(link.upperDimension()) != null || above(link.lowerDimension()) != null) {
            throw new IllegalArgumentException(
                    "A vertical boundary is already registered for " + link.id());
        }
        LINKS.add(link);
    }
