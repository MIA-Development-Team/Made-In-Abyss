...
final class CrossDimensionLodCacheIo implements AutoCloseable {
    ...
    CrossDimensionLodCacheIo(int capacity) {
        ...
        var queue = new PriorityBlockingQueue<Runnable>(capacity,
                Comparator.comparingInt((Runnable r) -> ((Job) r).read ? 0 : 1)
                        .thenComparingLong(r -> ((Job) r).sequence));
        executor = new ThreadPoolExecutor(1, 1, 0, TimeUnit.MILLISECONDS, queue, runnable -> {
    ...
    boolean submit(boolean read, Runnable action) {
        if (!slots.tryAcquire()) {
            ...
            Job victim = executor.getQueue().stream().map(r -> (Job) r).filter(job -> !job.read)
                    .max(Comparator.comparingLong(job -> job.sequence)).orElse(null);
    ...
    boolean hasWriteCapacity() { return slots.availablePermits() > 0; }
    ...
    private final class Job implements Runnable {
        ...
        Job(boolean read, long sequence, Runnable action) {
            this.read = read; this.sequence = sequence; this.action = action;
        }
