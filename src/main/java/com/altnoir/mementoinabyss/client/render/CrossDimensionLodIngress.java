...
final class CrossDimensionLodIngress {
    ...
    CrossDimensionLodIngress(int capacity, int byteCapacity, int retainedCapacity) {
        if (capacity <= 0 || byteCapacity <= 0 || retainedCapacity <= 0) throw new IllegalArgumentException("Invalid LOD mailbox capacity");
    ...
    synchronized void offer(CrossDimensionLodStreamPayload payload) {
        ...
        if (packets.size() >= capacity || size > byteCapacity - bytes || retained > retainedCapacity - retainedBytes) {
    ...
    synchronized Poll poll() {
        ...
        if (entry != null) { bytes -= entry.bytes; retainedBytes -= entry.retainedBytes; }
    ...
    synchronized int size() { return packets.size(); }
    synchronized int bytes() { return bytes; }
    synchronized int retainedBytes() { return retainedBytes; }
    synchronized void clear() { packets.clear(); bytes = retainedBytes = 0; rejected.clear(); epoch = 0; reset = false; }
