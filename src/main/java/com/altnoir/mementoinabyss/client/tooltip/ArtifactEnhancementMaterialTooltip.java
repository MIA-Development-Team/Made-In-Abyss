...
public final class ArtifactEnhancementMaterialTooltip {
                ...
                .forEach(
                        holder -> {
                            ...
                            updated.computeIfAbsent(
                                            recipe.material().value(), ignored -> new ArrayList<>())
                                    .add(recipe);
                        });
    ...
    public static void append(ItemTooltipEvent event) {
        List<ArtifactEnhancementRecipe> recipes =
                recipesByMaterial.get(event.getItemStack().getItem());
    ...
    public static Component modifierLine(ArtifactEnhancementRecipe recipe) {
        boolean percentage =
                recipe.operation() != AttributeModifier.Operation.ADD_VALUE
                        || MiaAttributes.CRITICAL_HIT
                                .unwrapKey()
                                .map(recipe.attribute()::is)
                                .orElse(false)
                        || MiaAttributes.CRITICAL_HIT_DAMAGE
                                .unwrapKey()
                                .map(recipe.attribute()::is)
                                .orElse(false);
        String range =
                formatValue(recipe.value().min(), percentage)
                        + " \u2013 "
