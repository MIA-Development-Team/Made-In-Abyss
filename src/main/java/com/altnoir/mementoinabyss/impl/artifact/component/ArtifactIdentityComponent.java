...
public record ArtifactIdentityComponent(ArtifactProfile profile) implements ArtifactItemComponent {
    ...
    public void appendTooltip(
        ...
        ArtifactProfile current =
                stack.getOrDefault(MiaDataComponents.ARTIFACT_PROFILE.get(), profile);
        tooltip.add(
                Component.translatable(
                                "tooltip.mementoinabyss.artifact.grade."
                                        + current.grade().getSerializedName())
                        .withStyle(current.grade().style()));
        tooltip.add(
                Component.translatable(
                                "tooltip.mementoinabyss.artifact.weight",
                                Component.literal(Integer.toString(current.weight()))
