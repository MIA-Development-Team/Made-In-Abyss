...
public record ArtifactEnhancement(int level, List<ArtifactAttribute> attributes) {
    ...
    public static final Codec<ArtifactEnhancement> CODEC =
            RecordCodecBuilder.create(
                    instance ->
                            instance.group(
                                            Codec.intRange(0, Integer.MAX_VALUE)
                                                    .fieldOf("level")
                                                    .forGetter(ArtifactEnhancement::level),
                                            ArtifactAttribute.CODEC
                                                    .listOf()
                                                    .fieldOf("attributes")
                                                    .forGetter(ArtifactEnhancement::attributes))
    ...
    public ArtifactEnhancement addAttribute(
        ...
        for (int i = 0; i < updated.size(); i++) {
            ...
            if (existing.attribute().equals(attribute)
                    && existing.modifier().operation() == operation) {
                updated.set(
                        i,
