...
public record ArtifactEnhancementRecipe(
    ...
    public static final MapCodec<ArtifactEnhancementRecipe> CODEC =
            RecordCodecBuilder.mapCodec(
                    instance ->
                            instance.group(
                                            Item.CODEC
                                                    .fieldOf("material")
                                                    .forGetter(ArtifactEnhancementRecipe::material),
                                            Codec.intRange(1, 64)
                                                    .fieldOf("count")
                                                    .forGetter(
                                                            ArtifactEnhancementRecipe
                                                                    ::materialCount),
                                            Attribute.CODEC
                                                    .fieldOf("attribute")
                                                    .forGetter(
                                                            ArtifactEnhancementRecipe::attribute),
                                            DoubleRange.CODEC
                                                    .fieldOf("value")
                                                    .forGetter(ArtifactEnhancementRecipe::value),
                                            AttributeModifier.Operation.CODEC
                                                    .fieldOf("operation")
                                                    .forGetter(
                                                            ArtifactEnhancementRecipe::operation))
                                    .apply(instance, ArtifactEnhancementRecipe::new));
    public static final StreamCodec<RegistryFriendlyByteBuf, ArtifactEnhancementRecipe>
            STREAM_CODEC =
                    StreamCodec.composite(
                            Item.STREAM_CODEC,
                            ArtifactEnhancementRecipe::material,
                            ByteBufCodecs.VAR_INT,
                            ArtifactEnhancementRecipe::materialCount,
                            Attribute.STREAM_CODEC,
                            ArtifactEnhancementRecipe::attribute,
                            DoubleRange.STREAM_CODEC,
                            ArtifactEnhancementRecipe::value,
    ...
    public ArtifactEnhancementRecipe {
        if (materialCount < 1 || materialCount > 64) {
            throw new IllegalArgumentException(
                    "Artifact enhancement material count must be between 1 and 64");
        }
    }

    @Override
    public boolean matches(ArtifactEnhancementRecipeInput input, Level level) {
        return ArtifactApi.canEnhance(input.artifact()) && isMaterial(input.material());
    }
    ...
    public Identifier modifierId() {
        String attributePath =
                attribute.unwrapKey().map(key -> key.identifier().getPath()).orElse("unknown");
        return MementoInAbyss.asResource(
