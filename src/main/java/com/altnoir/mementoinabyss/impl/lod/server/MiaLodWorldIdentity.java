...
final class MiaLodWorldIdentity {
    ...
    static synchronized UUID get(Object server, Path worldRoot) {
        Path file =
                worldRoot.toAbsolutePath().normalize().resolve("data/mementoinabyss/lod/world-id");
    ...
    private static UUID load(Path file) {
        try {
            ...
            LogUtils.getLogger()
                    .warn(
                            "Cannot persist LOD world identity; using a session-local cache"
                                    + " namespace",
                            failure);
