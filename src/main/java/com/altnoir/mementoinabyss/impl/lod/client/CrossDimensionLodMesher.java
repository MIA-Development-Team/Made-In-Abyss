...
import com.altnoir.mementoinabyss.impl.lod.client.CrossDimensionLodAmbientOcclusion.View;
...
final class CrossDimensionLodMesher {
    ...
    static CpuMesh build(CrossDimensionLodColumn payload, boolean ambientOcclusion) {
        ...
        AABB bounds =
                new AABB(
                        originX,
                        payload.minY() + payload.displayYOffset(),
                        originZ,
                        originX + 16,
                        payload.minY()
                                + payload.yCells() * payload.cellSize()
                                + payload.displayYOffset(),
                        originZ + 16);
        boolean ao = ambientOcclusion && payload.cellSize() <= 4;
        return new CpuMesh(
                payload.interiorCopy(ao),
                payload.sides(),
                bounds,
                payload.chunkX(),
                payload.chunkZ(),
    ...
    static QuadBuffer buildInterior(CrossDimensionLodColumn payload, boolean ao) {
        ...
        View view =
                ao && payload.cellSize() <= 4
                        ? new View(payload.chunkX(), payload.chunkZ(), 1, new Side[1][])
                                .withSource(payload)
                        : null;
    ...
    static void appendAoRim(QuadBuffer quads, CrossDimensionLodColumn source, View view) {
        if (source != null)
            new CrossDimensionLodMesher(source).meshInteriors(quads, view.withSource(source), true);
    }
    ...
    private void meshInteriors(QuadBuffer quads, View ao, boolean rimOnly) {
        ...
        for (int face = 0; face < 6; face++) {
            ...
            for (int plane = 0; plane < planes; plane++) {
                if ((face == 0 || face == 4) && plane == 0
                        || (face == 1 || face == 5) && plane == size - 1) continue;
                if (ao == null) {
    ...
    private void meshRegion(
        ...
        int ox = source.chunkX() * 16,
                oy = source.minY() + source.displayYOffset(),
                oz = source.chunkZ() * 16;
        ...
        for (int v = 0; v < height; v++)
            for (int u = 0; u < width; u++) {
                ...
                int x = face < 2 ? plane : su,
                        y = face >= 2 && face < 4 ? plane : sv,
                        z = face < 2 ? su : face < 4 ? sv : plane;
                ...
                int index = v * width + u;
                mask[index] =
                        state >= 0 && renderedState(source, sourceHeightField, nx, ny, nz) < 0
                                ? state + 1
                                : 0;
                if (light != null && mask[index] != 0)
                    light[index] =
                            CrossDimensionLodAmbientOcclusion.corners(
                                    ao,
                                    face,
                                    ox + (face < 2 ? 0 : u * cell),
                                    oy + (face >= 2 && face < 4 ? 0 : v * cell),
                                    oz + (face < 2 ? u * cell : face < 4 ? v * cell : 0),
                                    cell,
                                    cell);
            }
    ...
    record Side(int cellSize, int minY, int yCells, int[] palette, short[] cells) {
        ...
        int state(int along, int worldY) {
            ...
            int index =
                    cells.length == 1
                            ? 0
                            : cells.length == yCells ? y : along / cellSize * yCells + y;
        ...
        static Side capture(CrossDimensionLodColumn column, int side) {
            ...
            for (int u = 0; u < width; u++) {
                ...
                System.arraycopy(
                        column.voxels(), (z * width + x) * height, cells, u * height, height);
                if (column.cellSize() >= 8) {
                    ...
                    if (y >= 0 && y < height)
                        Arrays.fill(
                                ...
                                u * height + (direction > 0 ? height : y + 1),
                                cells[u * height + y]);
                }
            }
            ...
            return new Side(
                    column.cellSize(),
                    column.minY() + column.displayYOffset(),
                    height,
                    column.palette(),
                    cells);
        }
    ...
    static final class BoundaryBuilder {
        ...
        BoundaryBuilder(View ao) {
            this.ao = ao;
        }

        void append(
                QuadBuffer quads, int side, int chunkX, int chunkZ, Side source, Side neighbour) {
            if (source == null) return;
            int cell =
                    neighbour == null
                            ? source.cellSize
                            : Math.min(source.cellSize, neighbour.cellSize);
            int yStep = cell;
            if (neighbour != null)
                while (Math.floorMod(source.minY - neighbour.minY, yStep) != 0) yStep /= 2;
    ...
    private static void mergeMask(
        ...
        for (int v = 0; v < height; v++) {
            for (int u = 0; u < width; ) {
                ...
                int ao =
                        light == null
                                ? CrossDimensionLodAmbientOcclusion.UNOCCLUDED
                                : light[v * width + u];
                // A gradient may extend along its constant axis, never along the axis that changes
                // brightness.
                ...
                while (stretchU
                        && u + rectangleWidth < width
                        && mask[v * width + u + rectangleWidth] == value
                ...
                while (stretchV && v + rectangleHeight < height) {
                    for (int x = 0; x < rectangleWidth; x++) {
                        ...
                        if (mask[index] != value || light != null && light[index] != ao)
                            break heightLoop;
                    }
                    rectangleHeight++;
                }
                addGreedyQuad(
                        ...
                        u * cellU,
                        v * cellV,
                        rectangleWidth * cellU,
                        rectangleHeight * cellV,
                        value - 1,
                        ao,
                        quads);
    ...
    private static int renderedState(
            CrossDimensionLodColumn column, HeightField heights, int x, int y, int z) {
    ...
    static final class HeightField {
        ...
        private HeightField(
                int size, int[] topY, int[] topState, int[] bottomY, int[] bottomState) {
            this.size = size;
            this.topY = topY;
