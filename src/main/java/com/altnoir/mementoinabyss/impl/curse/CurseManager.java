...
public class CurseManager {
    ...
    public static void init(ServerStartedEvent event) {
        lookup = event.getServer().registryAccess().lookupOrThrow(CurseRegistries.CURSE);
    }

    public static Optional<CurseDimension> get(Identifier id) {
        return lookup.get(ResourceKey.create(CurseRegistries.CURSE, id)).map(Holder::value);
    }
}
