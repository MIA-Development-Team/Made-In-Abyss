package com.altnoir.mementoinabyss.content.artifact.client;
...
public final class ArtifactEnhancementScreen
    ...
    public ArtifactEnhancementScreen(
            ArtifactEnhancementMenu menu, Inventory inventory, Component title) {
    ...
    private void extractRecipeButtons(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        for (int index = startIndex; index < endIndex; index++) {
            ...
            graphics.blitSprite(
                    RenderPipelines.GUI_TEXTURED, sprite, x, y - 1, BUTTON_WIDTH, BUTTON_HEIGHT);
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (displayRecipes()) {
            ...
            for (int index = startIndex; index < endIndex; index++) {
                int visibleIndex = index - startIndex;
                double relativeX =
                        event.x() - (leftPos + RECIPES_X + visibleIndex % COLUMNS * BUTTON_WIDTH);
                double relativeY =
                        event.y() - (topPos + RECIPES_Y + visibleIndex / COLUMNS * BUTTON_HEIGHT);
                ArtifactEnhancementRecipe recipe = recipes.get(index).value();
                if (relativeX >= 0.0
                        && relativeY >= 0.0
                        && relativeX < BUTTON_WIDTH
            ...
            if (isScrollBarActive()
                    && event.x() >= scrollerX
                    && event.x() < scrollerX + SCROLLER_WIDTH
                    && event.y() >= scrollerY
                    && event.y() < scrollerY + SCROLLER_TRACK_HEIGHT) {
                scrolling = true;
                return true;
            }
        }
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        if (scrolling && isScrollBarActive()) {
            ...
            scrollOffset =
                    ((float) event.y() - trackTop - SCROLLER_HEIGHT / 2.0F)
