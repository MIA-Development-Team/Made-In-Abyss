package com.altnoir.mementoinabyss.content.rope.client;
...
public final class RopeLineRenderer {
    public static final Identifier TEXTURE = MementoInAbyss.asResource("textures/block/rope.png");
    ...
    public static void renderPoints(
            PoseStack.Pose pose, VertexConsumer consumer, float[] points, int light) {
        ...
        buildSegmentRings(points, coreStarts, coreEnds, overlayStarts, overlayEnds, tangents);
        ...
        for (int joint = 1; joint < pointCount - 1; joint++) {
            ...
            renderJointLayer(
                    ...
                    joint * 12,
                    points[centerOffset],
                    points[centerOffset + 1],
                    points[centerOffset + 2],
                    0.0F,
                    0.25F,
                    light);
            renderJointLayer(
    ...
    private static void buildSegmentRings(
        ...
        for (int segment = 0; segment < segmentCount; segment++) {
            ...
            fillRing(coreStarts, ringOffset, ax, ay, az, sx, sy, sz, ux, uy, uz, CORE_HALF_WIDTH);
            fillRing(coreEnds, ringOffset, bx, by, bz, sx, sy, sz, ux, uy, uz, CORE_HALF_WIDTH);
            fillRing(
                    overlayStarts,
                    ringOffset,
                    ax,
    ...
    private static void renderJointLayer(
        ...
        for (int side = 0; side < 4; side++) {
            ...
            float nx =
                    previousRing[previous]
                            + previousRing[previousNext]
                            + nextRing[following]
                            + nextRing[followingNext]
                            - centerX * 4.0F;
            float ny =
                    previousRing[previous + 1]
                            + previousRing[previousNext + 1]
                            + nextRing[following + 1]
                            + nextRing[followingNext + 1]
                            - centerY * 4.0F;
            float nz =
                    previousRing[previous + 2]
                            + previousRing[previousNext + 2]
                            + nextRing[following + 2]
                            + nextRing[followingNext + 2]
                            - centerZ * 4.0F;
            float inverseNormal = 1.0F / Math.max(1.0E-6F, length(nx, ny, nz));
            nx *= inverseNormal;
            ny *= inverseNormal;
            nz *= inverseNormal;
            vertex(pose, consumer, previousRing, previousNext, u1, 0.0F, light, nx, ny, nz);
            vertex(
                    pose,
                    consumer,
                    nextRing,
            ...
            vertex(
                    ...
                    ny,
                    nz);
            vertex(pose, consumer, previousRing, previous, u0, 0.0F, light, nx, ny, nz);
        }
    }
